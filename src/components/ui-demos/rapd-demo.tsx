    ...
    <div className="rounded-xl border border-white/10 bg-neutral-900/80 overflow-hidden w-full max-w-2xl mx-auto">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-white">RAPD Simulator</span>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm font-medium text-white mt-2">{steps[step].instruction}</p>
        </div>
        ...
        <div className="relative flex items-center justify-center gap-8 sm:gap-16 mb-6">
          ...
          <div className={`absolute transition-all duration-500 ${lightPos === 'left' ? 'left-[22%] sm:left-[28%]' : lightPos === 'right' ? 'right-[22%] sm:right-[28%]' : 'left-1/2 -translate-x-1/2'}`}>
            <div className={`h-3 w-3 rounded-full ${flashLeft || flashRight ? 'bg-yellow-300 shadow-lg shadow-yellow-300/50' : 'bg-neutral-600'} transition-all duration-300`} />
          </div>

          {/* Left Eye */}
          <div className="text-center">
            <div className={`w-24 h-20 sm:w-32 sm:h-24 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${flashLeft ? 'border-yellow-300/50 bg-yellow-300/5' : 'border-white/20 bg-white/5'}`}>
            ...
            <span className="text-xs font-medium text-white mt-2 block">Left Eye (Normal)</span>
            <span className="text-[10px] text-neutral-500">Constricts normally</span>
          </div>

          {/* Right Eye */}
          <div className="text-center">
            <div className={`w-24 h-20 sm:w-32 sm:h-24 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${flashRight ? 'border-yellow-300/50 bg-yellow-300/5' : 'border-white/20 bg-white/5'}`}>
              <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-full border border-white/10 flex items-center justify-center">
                <div
            ...
            <span className="text-xs font-medium text-white mt-2 block">Right Eye (Affected)</span>
            <span className={`text-[10px] ${step === 3 ? 'text-red-400' : 'text-neutral-500'}`}>{step === 3 ? 'RAPD Detected!' : 'Abnormal response'}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-2">
          {steps.map((_, i) => (
            <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'w-8 bg-cyan-400' : i < step ? 'w-4 bg-cyan-400/30' : 'w-4 bg-neutral-700'}`} />
          ))}
        </div>
      </div>
    </div>
